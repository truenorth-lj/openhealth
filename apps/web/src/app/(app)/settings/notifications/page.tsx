...
export default function NotificationSettingsPage() {
  ...
  const { data: savedCheckpoints } =
    trpc.notification.getMilestoneCheckpoints.useQuery();
  const { data: waterGoalData } = trpc.water.getGoal.useQuery();
  ...
  const saveCheckpoints =
    trpc.notification.saveMilestoneCheckpoints.useMutation({
      onSuccess: () => {
        utils.notification.getMilestoneCheckpoints.invalidate();
      },
      onError: (err) => toast.error(err.message),
    });
  ...
  const [reminderMode, setReminderMode] = useState<"interval" | "milestone">("interval");
  ...
  useEffect(() => {
    if (settings) {
      ...
      setReminderMode(settings.reminderMode);
  ...
  useEffect(() => {
    if (savedCheckpoints) {
      setCheckpoints(
        savedCheckpoints.map((cp) => ({ time: cp.time, targetMl: cp.targetMl }))
      );
    }
  }, [savedCheckpoints]);
  ...
  const handleSave = async () => {
    if (reminderMode === "milestone") {
      const errors = validateCheckpoints(checkpoints);
      if (errors.length > 0) {
        errors.forEach((key) =>
          toast.error(t(`settings:notificationsPage.${key}`))
        );
        return;
      }
    }
    ...
    if (reminderMode === "milestone") {
      await saveCheckpoints.mutateAsync(checkpoints);
    }
  };

  const addCheckpoint = () => {
    if (checkpoints.length >= 10) return;
    const lastTime = checkpoints.length > 0 ? checkpoints[checkpoints.length - 1].time : "09:00";
    const lastTarget = checkpoints.length > 0 ? checkpoints[checkpoints.length - 1].targetMl : 0;
    // Default: 2 hours after last, +500ml
    const [h, m] = lastTime.split(":").map(Number);
    const newH = Math.min(h + 2, 23);
    const newTime = `${String(newH).padStart(2, "0")}:${String(m).padStart(2, "0")}`;
    setCheckpoints([...checkpoints, { time: newTime, targetMl: lastTarget + 500 }]);
  };
  ...
  const updateCheckpoint = (index: number, field: keyof CheckpointDraft, value: string | number) => {
    const updated = [...checkpoints];
    updated[index] = { ...updated[index], [field]: value };
    setCheckpoints(updated);
  };

  const handleSuggestedConfig = () => {
    const goalMl = waterGoalData?.dailyTargetMl ?? 2500;
    const suggested = generateSuggestedCheckpoints(goalMl, startTime, endTime, 4);
    setCheckpoints(suggested);
  };

  const checkpointsChanged = (() => {
    if (!savedCheckpoints) return checkpoints.length > 0;
    if (savedCheckpoints.length !== checkpoints.length) return true;
    return savedCheckpoints.some(
      (cp, i) => cp.time !== checkpoints[i]?.time || cp.targetMl !== checkpoints[i]?.targetMl
    );
  })();
  ...
  const hasChanges =
    ...
    (enabled !== settings.enabled ||
      ...
      (reminderMode === "milestone" && checkpointsChanged));

  const isPending = updateSettings.isPending || saveCheckpoints.isPending;
    ...
    <div className="px-4 py-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
          <div className="flex items-center gap-3">
            <BellRing className="h-4 w-4 text-neutral-400 dark:text-neutral-600" strokeWidth={1.5} />
          ...
          <button
            ...
            className={`relative h-6 w-11 rounded-full transition-colors duration-300 ${
              enabled ? "bg-blue-500" : "bg-neutral-200 dark:bg-neutral-700"
            }`}
          >
            <span
          ...
          <div className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="space-y-3">
              <p className="text-xs font-light text-neutral-400">
                {t("notificationsPage.reminderMode")}
              </p>
              <div className="flex gap-2">
                {(["interval", "milestone"] as const).map((mode) => (
                  <button
                    key={mode}
                    onClick={() => setReminderMode(mode)}
                    className={`flex-1 rounded-lg border px-3 py-2 text-sm font-light transition-all duration-300 ${
                      reminderMode === mode
                  ...
                  >
                    {t(`notificationsPage.mode${mode.charAt(0).toUpperCase() + mode.slice(1)}`)}
                  </button>
                ))}
              </div>
            </div>
              ...
              <>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="mt-5 text-neutral-300 dark:text-neutral-700">—</span>
                    <div className="flex-1">
                      <label className="text-[10px] text-neutral-400 dark:text-neutral-600">
            ...
            {/* Milestone mode settings */}
            {reminderMode === "milestone" && (
              <div className="space-y-4">
                <p className="text-xs text-neutral-400 dark:text-neutral-600">
                  {t("notificationsPage.milestoneNote")}
                </p>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-light text-neutral-400">
                      {t("notificationsPage.milestoneCheckpoints")}
                    </p>
                    <button
                      onClick={handleSuggestedConfig}
                      className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-600 transition-colors"
                    >
                      <Sparkles className="h-3 w-3" strokeWidth={1.5} />
                      {t("notificationsPage.suggestedConfig")}
                    </button>
                  </div>
                  ...
                  {checkpoints.map((cp, index) => (
                    ...
                    >
                      <input
                        type="time"
                        value={cp.time}
                        onChange={(e) => updateCheckpoint(index, "time", e.target.value)}
                        className="w-28 rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2 text-sm font-light focus:outline-none focus:ring-1 focus:ring-blue-500"
                      />
                      <div className="flex-1 relative">
                        <input
                          ...
                          value={cp.targetMl}
                          onChange={(e) =>
                            updateCheckpoint(index, "targetMl", parseInt(e.target.value) || 0)
                          }
                          min={1}
                          max={10000}
                          className="w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2 pr-10 text-sm font-light focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                  ...
                  {checkpoints.length < 10 && (
                    <button
                      ...
                      className="flex items-center gap-2 w-full justify-center rounded-lg border border-dashed border-black/10 dark:border-white/10 py-2 text-sm font-light text-neutral-400 hover:text-foreground hover:border-foreground/20 transition-colors"
                    >
                      <Plus className="h-4 w-4" strokeWidth={1.5} />
                      {t("notificationsPage.addCheckpoint")}
                    </button>
                  )}

                  {checkpoints.length >= 10 && (
                    <p className="text-xs text-neutral-400 dark:text-neutral-600 text-center">
                      {t("notificationsPage.maxCheckpoints")}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
              ...
              <button
                onClick={() => setStopWhenGoalReached(!stopWhenGoalReached)}
                className={`relative h-6 w-11 rounded-full transition-colors duration-300 ${
                  stopWhenGoalReached ? "bg-blue-500" : "bg-neutral-200 dark:bg-neutral-700"
                }`}
              >
                <span
                  className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform duration-300 ${
                    stopWhenGoalReached ? "translate-x-5" : "translate-x-0"
                  }`}
                />
              </button>
            </div>
      ...
      {hasChanges && (
        ...
        >
          {isPending ? t("notificationsPage.saving") : t("notificationsPage.saveSettings")}
        </button>
      )}
    </div>
  );
